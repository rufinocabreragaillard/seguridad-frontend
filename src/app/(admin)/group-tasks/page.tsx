...
export default function PaginaTareasGrupo() {
  const t = useTranslations('groupTasks')
  ...
  const crud = useCrudPage<TareaGrupo, FormTarea>({
    ...
    camposBusqueda: (p) => [p.codigo_tarea_grupo, p.nombre_tarea_grupo, p.descripcion ?? '', p.tipo_acceso],
    formInicial: { nombre_tarea_grupo: '', descripcion: '', tipo_acceso: 'USUARIO', alias: '', ayuda: '' },
    itemToForm: (p) => ({
  ...
  const reordenar = async (nuevos: TareaGrupo[]) => {
    try {
      await tareasGrupoApi.reordenar(nuevos.map(p => ({ id_tarea_grupo: p.id_tarea_grupo, orden: p.orden ?? 0 })))
      crud.cargar()
    } catch { crud.cargar() }
  }
  ...
  return (
    <div className="relative flex flex-col gap-6 max-w-5xl">
      ...
      <div className="pr-28 flex items-start justify-between gap-4">
        <div>
          <PageHeader i18nNamespace="groupTasks" />
        </div>
      ...
      <TablaCrud
        ...
        onReordenar={(nuevos) => reordenar(nuevos as unknown as TareaGrupo[])}
