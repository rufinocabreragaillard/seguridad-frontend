...
export default function PaginaLogin() {
  ...
  const { login, loginConGoogle, loginConMicrosoft, loginConGithub, error } = useAuth()
  ...
  const [cargandoGithub, setCargandoGithub] = useState(false)
  ...
  const algunLoading = cargandoEmail || cargandoGoogle || cargandoMicrosoft || cargandoGithub
  ...
  const handleGithub = async () => {
    setErrorLocal('')
    setCargandoGithub(true)
    try {
      await loginConGithub()
    } catch (err) {
      setErrorLocal(err instanceof Error ? err.message : t('errorConGithub'))
      setCargandoGithub(false)
    }
  }
