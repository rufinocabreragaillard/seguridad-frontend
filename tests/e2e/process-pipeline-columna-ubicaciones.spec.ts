...
test.describe('process-pipeline — columna izquierda con ubicaciones', () => {
  ...
  test('muestra cabecera "Ubicaciones" en la columna izquierda', async ({ page }) => {
    await expect(page.getByText(/^ubicaciones$/i).first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test('muestra botón "Solo este directorio" habilitado', async ({ page }) => {
    const btn = page.getByRole('button', { name: /solo este directorio/i }).first();
    await expect(btn).toBeVisible({ timeout: 15000 });
    await expect(btn).toBeEnabled();
  });
  ...
  test('layout grid usa 3 columnas (md:grid-cols-3) en desktop', async ({ page }) => {
    ...
    await expect(page.getByText(/^ubicaciones$/i).first()).toBeVisible({ timeout: 15000 });
    // Localiza el contenedor que tiene las 3 columnas
