...
export const documentosApi = {
  ...
  subirTexto: (
    ...
    body: {
       ...
       *  DOCUMENTOS/DEBUG_TIEMPOS_PIPELINE='true'. Persistidos en
...
export const colaEstadosDocsApi = {
  ...
  inicializarPorEstado: (estado_origen: string, estado_destino: string, codigo_entidad?: string, tope?: number | null, codigo_ubicacion?: string | null, q?: string | null, codigo_proceso?: string | null, opciones?: Record<string, unknown> | null) =>
    api.post<{ encolados: number; omitidos: number; total: number }>('/cola-estados-docs/inicializar-por-estado', { estado_origen, estado_destino, codigo_entidad, tope: tope || null, codigo_ubicacion: codigo_ubicacion || null, q: q || null, codigo_proceso: codigo_proceso || null, opciones: opciones || null }).then((r) => r.data),
  cerrar: () =>
    api.post<{ eliminados: number }>('/cola-estados-docs/cerrar').then((r) => r.data),
  eliminar: (id: number) => api.delete(`/cola-estados-docs/${id}`),
  ejecutar: (estadoDestino?: string, codigoProceso?: string) =>
    api.post<{ mensaje: string; grupo: string; estado_destino?: string }>(
      '/cola-estados-docs/ejecutar',
      { estado_destino: estadoDestino || null, codigo_proceso: codigoProceso || null },
    ).then((r) => r.data),
...
export const ubicacionesDocsApi = {
  ...
  sincronizar: (datos: {
    codigo_entidad?: string
    // Modo nuevo: identificar raíz por path. Modo legacy: por código.
    ruta_completa_raiz?: string
    codigo_ubicacion_raiz?: string
    directorios: {
      // Modo nuevo: codigo_ubicacion null y backend autogenera. Modo legacy:
      // se respeta el valor si llega.
      codigo_ubicacion?: string | null
      nombre_ubicacion: string
      alias_ubicacion?: string
      codigo_ubicacion_superior?: string | null
      ruta_completa: string
      ruta_completa_superior?: string | null
      nivel: number
    }[]
  }) =>
    api.post<{ insertadas: number; deshabilitadas: number; actualizadas: number; total: number; excluidas: number }>('/ubicaciones-docs/sincronizar', datos).then((r) => r.data),
...
export const espaciosTrabajoApi = {
  ...
  actualizar: (id: number, datos: { nombre_espacio?: string; tipo_espacio?: string; fecha_termino?: string }) =>
